...
func (lbmap *LBBPFMap) UpsertMaglevLookupTable(svcID uint16, backends map[string]uint16, ipv6 bool, m uint64) error {
	...
	n := make([]uint16, m)
	copy(n, lbmap.maglevBackendIDsBuffer)
	fmt.Printf("chris n len=%v, cap=%v, m=%v\n", len(n), cap(n), m)

	// We don't need to pass down M into the Maglev-specific BPF maps because
	// it is only needed to be plumbed into the main service map
...
func updateMasterService(
	...
	if maglevTableSize != 0 { // Non-zero means Mavlev is enabled.
		// TODO(christarazi): we might need to change loadbalancer.BackendID to
		// uint32
