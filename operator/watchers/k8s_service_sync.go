...
import (
	...
	"github.com/cilium/cilium/pkg/k8s/watchers/callbacks"
...
var (
	...
	serviceCallbacks = callbacks.New()
)
...
func StartSynchronizingServices(shared bool, cfg ServiceSyncConfiguration) {
	...
	serviceCacheCallback := newServiceCacheCallback(swgSvcs, swgEps)
	serviceCallbacks.Register(serviceCacheCallback)
		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				...
				if k8sSvc := k8s.ObjToV1Services(obj); k8sSvc != nil {
					for _, f := range serviceCallbacks.Callbacks() {
						f.AddFunc(obj)
					}
				}
			},
